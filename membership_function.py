...
class MembershipFunction:
    ...
    def _increasing_function_to_membership_functions(self, increasing_function, horizontal_cut=0.5, overlap_factor=0.5):
        ...
        max_mf_left = np.max(membership_function_left)
        if max_mf_left == 0:
            max_mf_left = 1

        max_mf_right = np.max(membership_function_right)
        if max_mf_right == 0:
            max_mf_right = 1


        membership_function_left /= max_mf_left
        membership_function_right /= max_mf_right
    ...
    def _membership_function_activation(self, membership_function, check_value):
        ...
        # Return activation of membership functions using linear interpolation
        denominator = right_index - left_index
        if denominator == 0:
            denominator = 1

        return gf.lerp(membership_function[left_index], membership_function[right_index],
                       alpha=(check_value_transformed - left_index) / denominator)
